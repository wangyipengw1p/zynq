...
XIicPs psi2c0, psi2c1;

XV_demosaic vdmos0,vdmos1;
...
int transfer_data(const char *pData, int cam,int frame, int seq, int len);
...
//------------------------------------------------------------------------------------------------
//------------------------------------------------------------ functions for VDMA, demosaic and I2C

void init_cam(){
	i2c_init(&psi2c0, XPAR_PS7_I2C_0_DEVICE_ID, 40000);
	i2c_init(&psi2c1, XPAR_PS7_I2C_1_DEVICE_ID, 40000);
	sensor_init(&psi2c0);
	sensor_init(&psi2c1);
	sensor_set_output_size(&psi2c0, IMG_W, IMG_H);
	sensor_set_output_size(&psi2c0, IMG_W, IMG_H);
}

void init_demosaic(){
	XV_demosaic_Initialize(&vdmos0, XPAR_VIDEO_IN_0_V_DEMOSAIC_0_DEVICE_ID);
	XV_demosaic_Initialize(&vdmos1, XPAR_VIDEO_IN_1_V_DEMOSAIC_0_DEVICE_ID);
	XV_demosaic_Set_HwReg_width(&vdmos0,IMG_W);
	XV_demosaic_Set_HwReg_width(&vdmos1, IMG_W);
	XV_demosaic_Set_HwReg_height(&vdmos0,IMG_H);
	XV_demosaic_Set_HwReg_height(&vdmos1, IMG_H);
	XV_demosaic_EnableAutoRestart(&vdmos0);
	XV_demosaic_EnableAutoRestart(&vdmos1);
	XV_demosaic_Start(&vdmos0);
	XV_demosaic_Start(&vdmos1);
}
void init_vdma(){

	//Doing the following in order is important, please refer to the documentation for designflow
	InterruptConnect(&XScuGicInstance,XPAR_FABRIC_VIDEO_IN_0_AXI_VDMA_0_S2MM_INTROUT_INTR,XAxiVdma_WriteIntrHandler,(void *)&vdma_vin[0]);
	InterruptConnect(&XScuGicInstance,XPAR_FABRIC_VIDEO_IN_1_AXI_VDMA_0_S2MM_INTROUT_INTR,XAxiVdma_WriteIntrHandler,(void *)&vdma_vin[1]);
...
int main(void)
{
	...
	xil_printf("\r\n\r\n");
	xil_printf("-----Dual cam display server-----\r\n");
	...
	*/
	...
	while (1) {
		xemacif_input(netif);	//necessary for UDP
		...
		if(WriteOneFrameEnd[0] >= 0)
				{
					...
					for(int i=0;i<frame_length_curr;i+=UDP_BUFF_SIZE)
					{
						if((i+UDP_BUFF_SIZE)>=frame_length_curr)
						{
							...
							transfer_data((const char *)pFrames0[index]+i, 0,frame_id1, ++sn,cot);
						}
						else
						{

							transfer_data((const char *)pFrames0[index]+i, 0,frame_id1,++sn,UDP_BUFF_SIZE);
				...
				if(WriteOneFrameEnd[1] >= 0)
				{
					...
					for(int i=0;i<frame_length_curr;i+=UDP_BUFF_SIZE)
					{
						...
						{
							cot = frame_length_curr-i;
							transfer_data((const char *)pFrames1[index]+i,1,frame_id2,++sn, cot);
						}
						else
						{
							transfer_data((const char *)pFrames1[index]+i,1,frame_id2,++sn, UDP_BUFF_SIZE);
						}
					}
					//xil_printf("%d sec\n", time(NULL) - timeflag);
					WriteOneFrameEnd[1] = -1;
					if (frame_id2++ == 4) frame_id2 = 1;
				}
	}
